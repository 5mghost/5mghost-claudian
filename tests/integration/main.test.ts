...
describe('ClaudianPlugin', () => {
  ...
  describe('onload', () => {
    ...
    it('should migrate legacy cli path to hostname-based paths and clear old field', async () => {
      ...
      mockApp.vault.adapter.exists.mockImplementation(async (path: string) => {
        // claudeCliPath is now in 5mghost-claudian-settings.json
        return path === '.claude/5mghost-claudian-settings.json';
      });
      mockApp.vault.adapter.read.mockImplementation(async (path: string) => {
        if (path === '.claude/5mghost-claudian-settings.json') {
      ...
      const settingsWrite = (mockApp.vault.adapter.write as jest.Mock).mock.calls.find(
        ([path]) => path === '.claude/5mghost-claudian-settings.json'
      );
  ...
  describe('loadSettings', () => {
    it('should merge saved data with defaults', async () => {
      // Mock 5mghost-claudian-settings.json exists with custom values (Claudian-specific settings)
      mockApp.vault.adapter.exists.mockImplementation(async (path: string) => {
        return path === '.claude/5mghost-claudian-settings.json';
      });
      mockApp.vault.adapter.read.mockImplementation(async (path: string) => {
    ...
    it('should normalize blockedCommands when stored value is partial', async () => {
      // Mock 5mghost-claudian-settings.json exists with partial blockedCommands
      mockApp.vault.adapter.exists.mockImplementation(async (path: string) => {
        return path === '.claude/5mghost-claudian-settings.json';
      });
      mockApp.vault.adapter.read.mockImplementation(async (path: string) => {
    ...
    it('should reconcile model from environment and persist when changed', async () => {
      // Mock 5mghost-claudian-settings.json with environment variables
      mockApp.vault.adapter.exists.mockImplementation(async (path: string) => {
        return path === '.claude/5mghost-claudian-settings.json';
      });
      mockApp.vault.adapter.read.mockImplementation(async (path: string) => {
  ...
  describe('saveSettings', () => {
    it('should save settings to file', async () => {
      ...
      // Claudian-specific settings should be written to .claude/5mghost-claudian-settings.json
      expect(mockApp.vault.adapter.write).toHaveBeenCalledWith(
        '.claude/5mghost-claudian-settings.json',
      ...
      // Permissions are now in .claude/settings.json (CC format), not 5mghost-claudian-settings.json
  ...
  describe('loadSettings with conversations', () => {
    it('should load saved conversations from JSONL files', async () => {
      ...
      mockApp.vault.adapter.exists.mockImplementation(async (path: string) => {
        ...
        // 5mghost-claudian-settings.json exists
        if (path === '.claude/5mghost-claudian-settings.json') {
          return true;
        }
        return false;
    ...
    it('should clear session IDs when provider base URL changes', async () => {
      ...
      mockApp.vault.adapter.read.mockImplementation(async (path: string) => {
        if (path === '.claude/5mghost-claudian-settings.json') {
          // All these fields are now in 5mghost-claudian-settings.json
          return JSON.stringify({
